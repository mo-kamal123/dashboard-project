...
export const filterTable = (data, search, dataType = 'products') => {
    return data.filter((item) => {
        ...
        if (dataType === 'orders') {
            ...
            const totalMatch = item.total?.toString().toLowerCase().includes(searchLower);
            ...
            const priceMatch = item.price?.toString().toLowerCase().includes(searchLower);
            const stockMatch = item.stock?.toString().toLowerCase().includes(searchLower);
